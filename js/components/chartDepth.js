  ...
  ChartDepth = (function(_React$Component) {
    function ChartDepth(props) {
      ...
      var _this = _possibleConstructorReturn(
        this,
        (ChartDepth.__proto__ || Object.getPrototypeOf(ChartDepth)).call(
          this,
          props
        )
      );
      return (
        props.store.subscribe(function() {
          ...
          JSON.stringify(_this.state.orders) !== JSON.stringify(state.orders) &&
            _this.setState({
              orders: state.orders,
      ...
      _createClass(ChartDepth, [
        {
          key: "render",
          value: function() {
            if (!this.state.orders) return React.createElement("div", null);
            var orders = this.state.orders,
              buyOrders = orders.buys,
              sellOrders = orders.sells.slice().reverse(),
              depthData = [],
              median = 0;
            buyOrders.length > 0 && (median += buyOrders[0].price.toNumber()),
              sellOrders.length > 0 &&
                (median += sellOrders[sellOrders.length - 1].price.toNumber()),
              buyOrders.length > 0 && sellOrders.length > 0 && (median /= 2);
            for (var cumul = 0, i = 0; i < buyOrders.length; i += 1) {
            ...
            for (var _i = sellOrders.length - 1; _i >= 0; _i -= 1) {
              var _price = sellOrders[_i].price.toNumber(),
                _volume = Number(
                  this.weiToEth(
                    Math.abs(sellOrders[_i].availableVolume),
                    this.getDivisor(this.state.selectedToken)
                  )
                );
              depthData.push([_price, 0, cumul]),
                (cumul += _volume),
                0 === _i && depthData.push([1.1 * _price, 0, cumul]);
            }
            depthData.unshift([
            ...
            var depthDataFiltered = depthData.slice(0, 1).concat(
                depthData
                  ...
                  .filter(function(x) {
                    return x[0] > 0.025 * median && x[0] < 1.75 * median;
                  })
              ),
              options = {
                chartArea: {
