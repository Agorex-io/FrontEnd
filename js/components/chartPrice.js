  ...
  ChartPrice = (function(_React$Component) {
    function ChartPrice(props) {
      ...
      var _this = _possibleConstructorReturn(
        this,
        (ChartPrice.__proto__ || Object.getPrototypeOf(ChartPrice)).call(
          this,
          props
        )
      );
      return (
        props.store.subscribe(function() {
          ...
          JSON.stringify(_this.state.trades) !== JSON.stringify(state.trades) &&
            _this.setState({
              trades: state.trades,
      ...
      _createClass(ChartPrice, [
        {
          key: "render",
          value: function() {
            if (!this.state.trades) return React.createElement("div", null);
            ...
            start = new Date(36e5 * Math.round(start.getTime() / 36e5));
            var points = [],
              intervals = [["DAY", "val1", "val2", "val3", "val4"]];
            filteredRows.forEach(function(row) {
              ...
              if (date - start > 36e5) {
                ...
                (start = new Date(start.getTime() + 36e5)), (points = []);
              }
              points.push(point);
            });
            var options = {
