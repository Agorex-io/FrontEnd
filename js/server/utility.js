    ...
    return (
          ...
          try {
            "undefined" == typeof window
              ? fs.readFile(filename, { encoding: "utf8" }, function(
                  ...
                  err ? callback(err, void 0) : callback(void 0, data);
                })
              : utility.getURL(
                  window.location.origin + "/" + filename,
                  function(err, body) {
                    err ? callback(err, void 0) : callback(void 0, body);
                  }
                );
          } catch (err) {
            callback(err, void 0);
          ...
          config.etherscanAPIKey &&
            ...
            utility.getURL(url, function(err, body) {
              if (err) callback(err, void 0);
                ...
                try {
                  var result = JSON.parse(body),
                    functionAbi = contract.abi.find(function(element) {
                      return element.name === functionName;
                    }),
                    ...
                    safeResult = xss(sanitizer.sanitize(unsafeResult)),
                    finalResult =
                      "BigNumber" === unsafeResult.constructor.name
          ...
          if (web3In.currentProvider) {
            ...
            web3In.eth.call({ to: address, data: data }, function(err, result) {
              if (err) proxy(1);
                ...
                try {
                  var unsafeResult = solidityFunction.unpackOutput(result),
                    safeResult = xss(sanitizer.sanitize(unsafeResult)),
                    finalResult =
                      "BigNumber" === unsafeResult.constructor.name ||
                      "Number" === unsafeResult.constructor.name
                        ? new BigNumber(safeResult)
                        : safeResult;
                  callback(void 0, finalResult);
                } catch (errJson) {
      ...
      (utility.testSend = function(
        ...
        if (
          ...
          utils.isObject(args[args.length - 1]) && (options = args.pop()),
      ...
      (utility.send = function(
        ...
        var privateKey =
            privateKeyIn && "0x" === privateKeyIn.substring(0, 2)
              ...
              : privateKeyIn,
          args = Array.prototype.slice.call(argsIn).filter(function(a) {
            return void 0 !== a;
          }),
          options = {};
        "object" === _typeof(args[args.length - 1]) &&
          ...
          utility.getNextNonce(web3, fromAddress, function(err, nextNonce) {
            ...
            if (
              ...
              console.log(encodedParams), (options.data += encodedParams);
            ...
            try {
              ...
              if (web3.currentProvider && "MetaMask" === addrKind)
                ...
                ledgerEth
                  .signTransaction_async(config.ledgerPath, rawTx)
                  .then(function(result) {
                    var optionsPlugSig = Object.assign(options, {
                      ...
                      serializedTx = new Tx(optionsPlugSig)
                        .serialize()
                        .toString("hex");
                    post(serializedTx);
                  })
                ...
                utility.signTx(web3, fromAddress, _tx, privateKey, function(
                  errSignTx,
                  txSigned
                ) {
                  if (errSignTx)
      ...
      (utility.logsOnce = function(
        ...
        function decodeEvent(item) {
          ...
          if (eventAbis.length > 0) {
            ...
            return new SolidityEvent(web3, eventAbi, address).decode(item);
          }
        }
        function proxy(retries) {
          var url =
            ...
            utility.getURL(url, function(err, body) {
              if (err) callback(null, []);
                ...
                try {
                  ...
                  async.map(
                    ...
                    function(item, callbackMap) {
                      Object.assign(item, {
                        ...
                        callbackMap(null, decodeEvent(item));
                    },
                  ...
                  retries > 0 ? proxy(retries - 1) : callback(null, []);
                }
            });
        }
        proxy(1);
          ...
          config.etherscanAPIKey &&
            ...
            utility.getURL(url, function(err, body) {
              if (err) callback(err, void 0);
                ...
                var result = JSON.parse(body);
                callback(void 0, Number(utility.hexToDec(result.result)));
              }
            });
        }
        web3.currentProvider
      ...
      (utility.signTx = function(web3, address, txIn, privateKey, callback) {
        var tx = txIn;
        if (privateKey)
          tx.sign(new Buffer(privateKey, "hex")), callback(void 0, tx);
        else {
      ...
      (utility.sign = function(
        ...
        function prefixMessage(msgIn) {
          ...
          return (
            ...
            (msg = Buffer.concat([
              new Buffer("Ethereum Signed Message:\n" + msg.length.toString()),
              msg
            ])),
            (msg = web3.sha3("0x" + msg.toString("hex"), {
              encoding: "hex"
            })),
            "0x" + (msg = new Buffer(msg.slice(2), "hex")).toString("hex")
          );
        }
        ...
        if (web3.currentProvider && "MetaMask" === addrKind)
          web3.version.getNode(function(error, node) {
            node &&
              ...
              web3.eth.sign(address, msgToSign, function(err, sigResult) {
                if (err) callback("Failed to sign message", void 0);
                else {
                  ...
                  if (
                    ((msg =
                      node && (node.match("TestRPC") || node.match("MetaMask"))
                        ? new Buffer(msgToSign.slice(2), "hex")
                        : new Buffer(prefixMessage(msgToSign).slice(2), "hex")),
                    testSig(msg, sig))
                  ) {
          ...
          ledgerEth
            .signPersonalMessage_async(config.ledgerPath, msg.toString("hex"))
            .then(function(sigResult) {
              ...
              if (testSig(new Buffer(prefixedMessage.slice(2), "hex"), sig)) {
                var r = "0x" + sig.r.toString("hex"),
                  s = "0x" + sig.s.toString("hex"),
                  v = sig.v;
                callback(void 0, { r: r, s: s, v: v });
            ...
            .fail(function(err) {
              console.log(err), callback("Failed to sign message", void 0);
            });
        } else if (privateKeyIn) {
          var privateKey =
      ...
      (utility.verify = function(
        ...
        "0x" === r.substring(0, 2) && (r = r.substring(2, r.length)),
          "0x" === s.substring(0, 2) && (s = s.substring(2, s.length)),
        ...
        var pubKey = ethUtil.ecrecover(
          ...
          result =
            address ===
            "0x" +
              ethUtil.pubToAddress(new Buffer(pubKey, "hex")).toString("hex");
      ...
      (utility.toChecksumAddress = function(addrIn) {
        var addr = addrIn;
        return (
          addr && "0x" !== addr.substring(0, 2) && (addr = "0x" + addr),
      ...
      (utility.deployContract = function(
        ...
        utility.readFile(sourceFile + ".bytecode", function(
          ...
          utility.readFile(sourceFile + ".interface", function(
                    ...
                    function(callbackWhilst) {
                      setTimeout(function() {
                        utility.txReceipt(web3, txHash, function(err, receipt) {
                          receipt && (contractAddr = receipt.contractAddress),
                            callbackWhilst(null);
                        });
                      }, 1e3);
                    },
            ...
            } else callback("Could not load bytecode and ABI", void 0);
          });
        });
      }),
      ...
      (utility.hexToDec = function(hexStrIn, length) {
        var hexStr = hexStrIn;
        if (
          ("0x" === hexStr.substring(0, 2) && (hexStr = hexStr.substring(2)),
      ...
      (utility.pack = function(dataIn, lengths) {
        for (
          ...
          "string" == typeof data[i] && "0x" === data[i].substring(0, 2)
      ...
      (utility.unpack = function(str, lengths) {
        for (var data = [], length = 0, i = 0; i < lengths.length; i += 1)
          (data[i] = parseInt(
            utility.hexToDec(str.substr(length, lengths[i] / 4), lengths[i]),
            10
          )),
            (length += lengths[i] / 4);
        return data;
      ...
      (utility.convertBase = function(str, fromBase, toBase) {
        ...
        for (var outArray = [], power = [1], i = 0; i < digits.length; i += 1)
          digits[i] &&
      ...
      (utility.add = function(x, y, base) {
        for (
          ...
          var zi =
            carry + (i < x.length ? x[i] : 0) + (i < y.length ? y[i] : 0);
          z.push(zi % base), (carry = Math.floor(zi / base)), (i += 1);
        }
        return z;
      }),
      ...
      (Array.prototype.getUnique = function() {
        for (var u = {}, a = [], i = 0, l = this.length; i < l; ++i)
          u.hasOwnProperty(this[i]) || (a.push(this[i]), (u[this[i]] = 1));
        return a;
      }),
      ...
      (Array.prototype.equals = function(b) {
        ...
        for (var i = 0; i < this.length; ++i) if (this[i] !== b[i]) return !1;
        return !0;
      }),
      (Math.sign =
        Math.sign ||
